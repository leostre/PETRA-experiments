...
class TemplateLoader:
    ...
    def load_dataset(self):
        if self.problem == 'classification':
            try:
                ...
                if DATASET_PROP:
                    train_loader['part'] = DATASET_PROP
                    test_loader['part'] = DATASET_PROP
    ...
    def get_peft_config(self, peft_dict):
        ...
        if ptype == 'pruning':
            return ptype, PruningTemplate(importance='magnitude', pruning_ratio=0.5, finetune_params=deepcopy(base_config))
        if ptype == 'quantization':
            return ptype, QuantTemplate(quant_type=qtype, allow_emb=False, allow_conv=True, qat_params=deepcopy(base_config))
    ...
    for task_name, task in task_problems.items():
        ...
        for combo_id, combo in enumerate(all_valid_combos):
            ...
            if loader.dataset == 'Imagenette':
                tp = 'imagenet'
            elif loader.dataset == 'AppliancesEnergy':
                tp = 'rgr'
            ...
            for step, peft_key in enumerate(combo):
                ...
                try:
                    with redirect_stdout(buffer), redirect_stderr(buffer):
                        ...
                        config = ConfigFactory.from_template(api_template, name=f'CONF{combo_id}')()
